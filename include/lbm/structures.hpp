...
typedef struct lbm_mesh_type_s {
  ...
  /// Cached x coordinates of non-fluid cells processed by special_cells().
  uint32_t* special_x;
  /// Cached y coordinates of non-fluid cells processed by special_cells().
  uint32_t* special_y;
  /// Cached type of each non-fluid cell processed by special_cells().
  lbm_cell_type_t* special_type;
  /// Number of cached non-fluid cells.
  size_t special_count;
  /// Cache state. 0 means not built yet, 1 means ready.
  int special_ready;
} lbm_mesh_type_t;
