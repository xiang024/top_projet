...
void open_data_file(lbm_data_file_t* file, const char* fname) {
  ...
  // Allocate memory
  file->entries = static_cast<lbm_file_entry_t*>(
    malloc(file->header.mesh_height * file->header.mesh_width * sizeof(lbm_file_entry_t))
  );
}
...
bool read_next_frame(lbm_data_file_t* file) {
  ...
  // Load the frame
  size_t res
    = fread(file->entries, sizeof(lbm_file_entry_t), file->header.mesh_height * file->header.mesh_width, file->fp);
...
bool seek_to_frame(lbm_data_file_t* file, int frame) {
  int res
    = fseek(file->fp, frame * sizeof(lbm_file_entry_t) * file->header.mesh_height * file->header.mesh_width, SEEK_CUR);
  return (res == 0);
}
  ...
  for (size_t i = 0; i < file->header.mesh_width; i++) {
    for (size_t l = 0; l < file->header.lines; l++) {
      for (size_t j = 0; j < line_height; j++) {
        size_t pos = line_height * i + j + l * line_height * file->header.mesh_width;
        printf("%zu %zu %f %f\n", i, j + l * line_height, file->entries[pos].rho, file->entries[pos].v);
      }
    }
    printf("\n");
  }
